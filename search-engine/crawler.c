...
void get_Page(char* argv[])  // function to fetch url from user and contact in urlBuffer and fetch page source code and add it to temp file
{
    char file_name[] = "temp.txt";
    char urlBuffer[urlLength + 300] = {0};
    strcat(urlBuffer, "wget -O ");
    strcat(urlBuffer, "./temp.txt ");
    strcat(urlBuffer, argv[1]);
    system(urlBuffer);
    system("clear");
    printf("Page fetched successfully");
    char *links;
    char *html=load_file(file_name);
    int ans=0;
    for(int i=0;i<10;i++)
    {
      ans = GetNextURL(html,argv[1],links,ans);
      printf("%s\n",links);
    }
}
